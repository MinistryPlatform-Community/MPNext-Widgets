...
Item 15 is **done (2026-09-08), by deletion rather than by re-hashing.** The
finding stood: `dist/atcb.min.js` does not exist in the npm tarball, so jsDelivr
auto-minified `dist/atcb.js` on the fly and `ATCB_SRI` covered CDN-generated
bytes rather than author-published ones. Neither option the file proposed was
taken, because a third fact outranked both — `add-to-calendar-button` is
**ELv2-licensed** (not open source, and v3 is the paid line), and what it
contributed was a dropdown over four query-string templates and an `.ics` file.
...
`loadScript(url, integrity?)` sets `integrity` + `crossorigin="anonymous"`. Only
`FC_SRI` remains, beside its version constant, now that item 15 removed the
add-to-calendar CDN script. **Bumping the FullCalendar CDN version requires
recomputing its hash in the same edit** (see item 7).
