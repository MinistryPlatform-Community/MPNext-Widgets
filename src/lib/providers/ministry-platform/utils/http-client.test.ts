...
describe('HttpClient', () => {
  ...
  describe('POST FormData Requests', () => {
    it('should make POST request with FormData (no Content-Type header)', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.ministryplatform.com/files',
        {
          ...
          headers: {
            'Authorization': 'Bearer test-access-token-123',
            'Accept': 'application/json',
            // No Content-Type for FormData - lets browser set boundary
          },
          body: formData,
        }
      );
  ...
  describe('PUT FormData Requests', () => {
    it('should make PUT request with FormData (no Content-Type header)', async () => {
      const formData = new FormData();
