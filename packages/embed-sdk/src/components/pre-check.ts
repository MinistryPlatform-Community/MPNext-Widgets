...
export class PreCheckWidget extends MPNextWidget {
  ...
  static get observedAttributes(): string[] {
    return ["event-date", "read-query-string", "allow-date-picker", "show-qr"];
  }
  ...
  /** The check-in code's SVG markup, once fetched. Only with `show-qr`. */
  private qrSvg: string | null = null;
  /** The QR fetch failed. Rendered as a note, never as the widget's error state. */
  private qrFailed = false;
  ...
  /**
   * Render the check-in QR code. **Off unless the church opts in.**
   *
   * The payload predates MP's `Events.Allow_QR_Check_In` / `QR_Redirect_Url`
   * columns, which are a different (URL-redirect) mechanism, and whether a
   * current check-in station still scans the older barcode cannot be verified
   * from any repo — only at a physical station. So a church turns this on once
   * it has confirmed its own station reads `pre|M/d/yyyy|householdId`, and
   * every other church gets a working widget instead of a dead image that
   * support has to explain. The pre-check submission carries the value on its
   * own: it writes the `Event_Participants` rows that put the family on the
   * station's expected list, which is what actually shortens the queue.
   */
  private get showQr(): boolean {
    return this.boolAttr("show-qr");
  }
  ...
  private async load(): Promise<void> {
    ...
    try {
      ...
      // Fetched after the list, and awaited, so the panel and the rows paint
      // together rather than the code popping in a beat later. A failure here
      // is a note inside the panel, never the widget's error state: the code
      // is a convenience on top of a submission that works without it.
      if (this.showQr) await this.loadQr(data.eventDate);
  ...
  /**
   * Fetch the check-in code for a date.
   *
   * The household id is **not** sent: the route takes it from the session, so
   * a code can only ever be minted for the caller's own household.
   */
  private async loadQr(eventDate: string): Promise<void> {
    this.qrSvg = null;
    this.qrFailed = false;

    try {
      const res = await this.fetch(
        `/api/embed/pre-check/qr?eventDate=${encodeURIComponent(eventDate)}`,
      );
      if (this.detached) return;
      ...
      const payload = (await res.json().catch(() => ({}))) as { svg?: string };
      // The markup is generated by this repo's own encoder from a string the
      // server built — no caller text reaches it — so setting it into the
      // shadow root is not an untrusted-HTML question. It is still shape-checked
      // here rather than trusted blindly.
      this.qrSvg = typeof payload.svg === "string" && payload.svg.startsWith("<svg")
        ? payload.svg
        : null;
      this.qrFailed = this.qrSvg === null;
    } catch {
      if (this.detached) return;
      this.qrFailed = true;
    }
  ...
  private renderState(): string {
    switch (this.state) {
      ...
      case "no-events":
        ...
        return `${this.renderDateControl()}${this.renderQr()}${this.renderNotices()}${this.statusRegion(
      ...
      case "list":
        return `${this.renderDateControl()}${this.renderQr()}${this.renderNotices()}${this.renderMembers()}${this.renderActions()}`;
  ...
  /**
   * The check-in code panel. Empty markup unless `show-qr` is on.
   *
   * A failed fetch renders a note *inside* the panel rather than taking over
   * the widget: the submission is the part that puts a family on the station's
   * expected list, and it works whether or not a code was drawn.
   */
  private renderQr(): string {
    if (!this.showQr) return "";

    const body = this.qrSvg
      ? `<div class="pc-qr-code">${this.qrSvg}</div>
         <p class="pc-qr-help">${this.escapeHtml(this.t("preCheck.qrHelp"))}</p>`
      : this.qrFailed
        ? `<p class="pc-msg">${this.escapeHtml(this.t("preCheck.qrUnavailable"))}</p>`
        : "";

    return `
      <section class="pc-qr">
        <h3 class="pc-qr-title">${this.escapeHtml(this.t("preCheck.qrTitle"))}</h3>
        ${body}
      </section>`;
  }
